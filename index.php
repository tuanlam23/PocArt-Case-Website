...
<div class="ws_images"><ul>
	<?php 
						...
						while($row = mysqli_fetch_array($sql)){
			 			...
			 			$query_link = mysqli_query($conn,"SELECT * FROM sanpham WHERE img = '$logo1'");
			 			if(isset($query_link)){
							while($get_link = mysqli_fetch_array($query_link)){
							$link = $get_link['ID_SP'];
						} 
					...
					?>
		<li><a href="view/product.php?id=<?=$link?>" target="_blank" style="color: #535353"><img src="img/<?= $logo1?>" alt="Sản phẩm hot" title="Sản phẩm hot" id="wows1_0"/></a></li>
		<?php 
						...
						$link2 = 20;
$query_link2 = mysqli_query($conn,"SELECT * FROM sanpham WHERE img = '$logo2'");
			 			if(isset($query_link2)){
							while($get_link2 = mysqli_fetch_array($query_link2)){
							$link2 = $get_link2['ID_SP'];
						} 
						
						}
					?>
		<li><a href="view/product.php?id=<?=$link2?>" target="_blank" style="color: #535353"><img src="img/<?= $logo2?>" alt="Sản phẩm hot" title="Sản phẩm hot" id="wows1_1"/></a></li>
		<?php 
						$query_get_img = "SELECT * FROM banner WHERE ten_banner = 'center_banner_3'";
						$sql = mysqli_query($conn,$query_get_img);
						while($row = mysqli_fetch_array($sql)){
							$logo3 = $row['img_banner'];
						}$query_link3 = mysqli_query($conn,"SELECT * FROM sanpham WHERE img = ".$logo3);
			 			$link3 = 20;
$query_link3 = mysqli_query($conn,"SELECT * FROM sanpham WHERE img = '$logo3'");
			 			if(isset($query_link3)){
							while($get_link3 = mysqli_fetch_array($query_link3)){
							$link3 = $get_link3['ID_SP'];
						} 
						
						}
					?>
		<li><a href="view/product.php?id=<?=$link3?>" target="_blank" style="color: #535353"><img src="img/<?= $logo3?>" alt="Sản phẩm hot" title="Sản phẩm hot" id="wows1_2"/></a></li>
	</ul></div>
...
<div class="content">
	<div class="container">
		<div class="content-top">
			...
			<!--Chọn theo concept-->
			<?php
				$query_get_categories = mysqli_query($conn,"SELECT * FROM categories");
			 	while($row_categories = mysqli_fetch_array($query_get_categories)){
					$id_cate = $row_categories['id_cate'];
					$name_cate = $row_categories['name_cate'];
					$query_get_so_luong = mysqli_query($conn,"SELECT * FROM sanpham WHERE id_cate = '$id_cate'");
					$so_luong = mysqli_num_rows($query_get_so_luong);
					while($row_img_cate = mysqli_fetch_array($query_get_so_luong)){
						$img_cate = $row_img_cate['img'];
						if($img_cate != ''){
							break;
						}
					}
					...
					<div class="col-md-3 col-md2" >
								 <div >
							<a href="concept.php?id_cate=<?= $id_cate?>" style="color: #535353;text-decoration: none;text-decoration-color: ">
							
							<div style="background-color: rgba(249,249,249,1.00);color: rgba(85,85,85,1.00);"><img class="img-responsive" src="img/<?= $img_cate == ''? 'case_empty.png':"$img_cate" ?>" />
								<p style="font-size: 20px; margin-top: 10px" align="center"><b><?= $name_cate?></b></p>
								<p align="center" style="font-size: 10px; margin-bottom: 30px;"><?= $so_luong?> SẢN PHẨM<br>&nbsp;</p>
							</div>
