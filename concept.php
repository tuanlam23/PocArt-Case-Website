...
<div class="container">
	<!--Lấy tên concept-->
		<?php
			$id_cate_in = $_GET['id_cate'];
			$query_get_name_cate = mysqli_query($conn,"SELECT * FROM categories WHERE id_cate = $id_cate_in");
			while($row_name_cate = mysqli_fetch_array($query_get_name_cate)){
				$name_cate = $row_name_cate['name_cate'];
			}
		?>
	<!---->
	<br><br>
	<a style="color: #535353" href="index.php">TRANG CHỦ </a>/ <a style="color: #535353"><b><?= $name_cate?></b></a>
		...
		<div class="content-top">
			...
			<!--Phân trang-->
				<?php
					$query_get_all = mysqli_query($conn,"SELECT * FROM sanpham WHERE id_cate = $id_cate_in ORDER BY date_in DESC");
					$so_item  = mysqli_num_rows($query_get_all);
			 		...
			 		if(isset($_GET['page'])){
						$from = ($_GET['page']-1)*8;
			 			$query_get_8 = mysql_query("(SELECT * FROM sanpham WHERE id_cate = $id_cate_in ORDER BY ID_SP DESC) LIMIT $from, 8");
						...
						$query_get_8 = mysqli_query($conn,"(SELECT * FROM sanpham WHERE id_cate = $id_cate_in ORDER BY ID_SP DESC) LIMIT 0, 8");
...
<?php
		...
		for($t = 1; $t<= $so_trang; $t++){
				if($t == $_GET['page']){
							...
							echo"<a href='concept.php?id_cate=$id_cate_in&page=$t' style='color: #535353'>Trang $t</a><a>&nbsp|&nbsp</a>";
						}
						}}
		else{
			echo("<b>Trang 1 </b>");
